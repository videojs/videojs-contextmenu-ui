...
function onVjsContextMenu(e) {
  ...
  // APIs... :(
  cmui.closeMenuListener = _videoJs2['default'].bind(this, function (ee) {
    _videoJs2['default'].log('contextmenu-ui: closeMenuListener', ee);
    _videoJs2['default'].off(_globalDocument2['default'], ['mousedown', 'touchstart'], cmui.closeMenuListener);
  ...
  _videoJs2['default'].on(_globalDocument2['default'], ['mousedown', 'touchstart'], cmui.closeMenuListener);
}
...
function contextmenuUI(options) {
  ...
  this.on('vjs-contextmenu', cmui.onVjsContextMenu).on('dispose', playerIsDisposing).ready(function () {
    return _this2.addClass(CLASS_PREFIX);
  });
}

_videoJs2['default'].plugin('contextmenuUI', contextmenuUI);
contextmenuUI.VERSION = '3.0.0';
